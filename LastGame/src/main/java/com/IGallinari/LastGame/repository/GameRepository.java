...
public interface GameRepository extends JpaRepository<Game,Integer> {
    ...
    List<Integer> findAllGameIdsBeforeDate(@Param("inputDate") LocalDate date);

    @Query(value = "SELECT g.id FROM game g WHERE (g.id NOT IN(SELECT sp.idGame FROM statsplayer sp GROUP BY sp.idGame) OR g.id NOT IN(SELECT sg.idGame FROM statsgame sg GROUP BY sg.idGame)) AND g.status=3 AND g.date<current_date AND g.season=:inputSeason", nativeQuery = true)
    List<Integer> findAllGameIdsBeforeDateNotCompleted(@Param("inputSeason") int season);
